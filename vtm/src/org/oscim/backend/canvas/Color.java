...
public final class Color {
    ...
    public static int parseColor(String colorString) {
        if (colorString.charAt(0) == '#') {
            return parseArgbColor(colorString.substring(1));
        }
    ...
    public static int parseArgbColor(String pureColorString) {
        // Use a long to avoid rollovers on #ffXXXXXX
        long color = Long.parseLong(pureColorString, 16);
        if (pureColorString.length() == 6) {
            // Set the alpha value
            color |= 0x00000000ff000000;
        } else if (pureColorString.length() != 8) {
            parseColorException(pureColorString);
        }
        return (int) color;
    }
