...
public class TextureItem extends Inlist<TextureItem> {
    ...
    static final private ArrayList<Integer> textureIdCache = new ArrayList<>();
    ...
    public static class TexturePool extends SyncPool<TextureItem> {
        ...
        private void uploadTexture(TextureItem t) {
            ...
            if (t.id < 0) {
                if (textureIdCache.isEmpty()) {
                    int[] textureIds = GLUtils.glGenTextures(1);
                    t.id = textureIds[0];
                } else {
                    int lastIndex = textureIdCache.size() - 1;
                    t.id = textureIdCache.get(lastIndex);
                    textureIdCache.remove(lastIndex);
                }
        ...
        synchronized (disposedTextures) {
            ...
            if (size > 0) {
                for (int i = 0; i < size; i++) {
                    textureIdCache.add(disposedTextures.get(i).intValue());
                }
//                int[] tmp = new int[size];
//                for (int i = 0; i < size; i++) {
//                    tmp[i] = disposedTextures.get(i).intValue();
//                    log.debug("dispose {}", tmp[i]);
//                }
//
